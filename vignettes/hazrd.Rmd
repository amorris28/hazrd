...
```{r}
OR80_20 = get_or(test_data, or_age = 70, conf.int = TRUE, bootstrap.iterations = 300)
str(OR80_20)
```
...
95% confidence intervals for the estimates from the `get_hr`, `get_or`,  and `get_cindex` functions can be generated by bootstrapping. This is done by by specifying `conf.int = TRUE` and providing a number of bootstrap iterations with `bootstrap.iterations = `. In this case, `get_hr` returns a list containing the hazard ratio and the values for the upper and lower 95% confidence intervals as well as all of the HR estimates from each bootstrap iteration in `iters`.
...
HR80_20 = get_hr(combined_data, conf.int = TRUE, bootstrap.iterations = 300)
ages = c(60, 70, 80)
OR80_20 = lapply(ages, function(x) {get_or(combined_data, or_age = x, conf.int = TRUE, bootstrap.iterations = 300)})
c_index = get_cindex(combined_data, conf.int = TRUE, bootstrap.iterations = 300)
