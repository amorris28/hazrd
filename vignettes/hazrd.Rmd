...
In the literature on PHS, there are at least two ways that folks visualize the survival estimates for different PHS models. One is to compare the 20th percentile to the 80th percentile. The other is to compare quintiles, that is, the top 20% of PHSes to the bottom 20% (approximately equal to the 10th versus 20th percentile). Here is an explanation of how to do both and how those estimates compare:

## Percentiles

```{r}
library(survival)
# Fit the coxph model and generate a survival curve
...
# Using out original model, generate a predicted survival curve
...
# Fit the modell, this time with group as the predictor rather than the PHS
...
Here, the blue curves are the quintile curves or group-based curves and the red curvies are the percentile-based curves.
